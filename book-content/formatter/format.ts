...
const ensureExercisesAndSolutionsHaveCorrectNumbers = (currentLine: Line) => {
    if (
        ...
        const { contentStartIndex, endIndex } =
            getContentOfHeading(currentLine);

        decoratedLines
            .slice(contentStartIndex, endIndex)
            .filter((line) => {
                return (
                    line.isHeading() && line.getContent().startsWith("Exercise")
                );
            })
            .map((exercise, index) => {
        ...
        decoratedLines
            ...
            .filter((line) => {
                return (
                    line.isHeading() && line.getContent().startsWith("Solution")
                );
            })
            .map((solution, index) => {
